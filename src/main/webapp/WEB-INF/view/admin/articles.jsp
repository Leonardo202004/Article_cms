		...
		<div class="form-inline">
			<div class="form-group">
				<label for="title">标题</label> <input class="form-control" id="title"
					type="text" name="title" value="${articles.title}">
			</div>

			<div class="form-check">
				<input class="form-check-input" type="radio" name="status"
					id="status0" value="0" ${articles.status==0?"checked":"" }>
				<label class="form-check-label" for="status0"> 待审 </label>
			</div>
			<div class="form-check">
				<input class="form-check-input" type="radio" name="status"
					id="status1" value="1" ${articles.status==1?"checked":"" }>
				<label class="form-check-label" for="status1"> 已审 </label>
			</div>
			<div class="form-check">
				<input class="form-check-input" type="radio" name="status"
					id="status-1" value="-1" ${articles.status==-1?"checked":"" }>
				<label class="form-check-label" for="status-1"> 驳回 </label>
			</div>
			<div>
				...
				<c:forEach items="${info.list}" var="article" varStatus="i">
					...
					<tr>
						...
						<td><button type="button" class="btn btn-link "
								...
								onclick="detail(${article.id})">详情</button></td>
					</tr>
				</c:forEach>
				<tr>
...
function query(){
	...
	$("#center").load("/admin/list?title="+title+"&status="+status);
	
}
function goPage(pageNum){
	$("#center").load("/admin/list?pageNum="+pageNum);
}
//设置热门文章
function update(id,obj){
	...
	$.post("/admin/updateArticle",{id:id,hot:hot},function(msg){
		if(msg>0){//更新成功
			//1 改变按钮的内容
...
function check(status){
	...
	$.post("/admin/updateArticle",{id:articleId,status:status},function(msg){
		if(msg>0){
		  $("#msg").text("操作成功");	
		}else{
			 $("#msg").text("操作失败");	
