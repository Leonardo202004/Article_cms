	...
	<div class="container">
		<div class="form-group form-inline">
			...
			<div class="form-check ml-2">
				...
				<button class="btn btn-info btn-sm ml-2" type="button"
				...
				<c:forEach items="${info.list}" var="user">
					<tr>
						...
						<td><c:if test="${user.locked==0}">
								...
								<button class="btn btn-warning btn-sm" type="button" onclick="update(${user.id},this)">禁用</button>
	...
	<script type="text/javascript">
		...
		function update(id,obj){
			var locked =$(obj).text()=='正常'?1:0
					...
					$.post("/admin/updateUser",{id:id,locked:locked},function(msg){
						if(msg>0){//更新成功
							//1 改变按钮的内容
							$(obj).text($(obj).text()=='正常'?'禁用':'正常');
						//2改变按钮的演示
						   $(obj).attr("class",$(obj).text()=='正常'?'btn btn-success btn-sm':'btn btn-warning btn-sm');
